...
'//Add draft
If InStr(1, xDraftArr(xCntr), " [•]") Then
xDraftArr(xCntr) = Replace(xDraftArr(xCntr), " [•]", vbNullString): xDraftArr(xCntr) = xDraftArr(xCntr) & " [•]"
Else
xDraftArr(xCntr) = Replace(xDraftArr(xCntr), " [...]", vbNullString): xDraftArr(xCntr) = xDraftArr(xCntr) & " [...]"
End If
...
xTime = Replace(ETWEETXLPOST.RuntimeHdr.Caption, "Runtime", vbNullString)
xTime = xTime & " " & xRuntimeArr(xCntr)
Range("Runtime").Offset(xCntr, 0).Value = xTime
...
If Range("DataPullTrig").Value2 = 0 Then
...
'//Cleanup
Call clnProf
If Range("DataPullTrig").Value2 = 1 Then Exit Sub '//Handle triggering twice...
'//clear post boxes...
...
Call clnTwt
If xType = 0 Then
                  xDraft = Replace(ETWEETXLPOST.DraftBox.Value, " [•]", vbNullString)
                Else
                  xDraft = Replace(ETWEETXLPOST.DraftBox.Value, " [...]", vbNullString)
                    End If
...
If Range("HoverPos").Value2 = 44 Then

lastR = Cells(Rows.Count, "P").End(xlUp).Row

For X = 1 To lastR

If InStr(1, xTwt, Range("Draftlink").Offset(X, 0).Value2) Then
If X = ETWEETXLQUEUE.QueueBox.ListIndex + 1 Then
    
If Range("xlasWinForm").Value2 <> 11 Then
    If Range("xlasWinForm").Value2 <> 14 Then
        If ETWEETXLPOST.ProfileListBox.Value <> Range("ProfileLink").Offset(X, 0).Value Then _
           ETWEETXLPOST.ProfileListBox.Value = Range("ProfileLink").Offset(X, 0).Value
           Range("xlasWinForm").Value2 = 14: Call getWindow(xWin)
                GoTo FindDraft
                    Else
