...
Public Sub ScreenDefault()

On Error Resume Next

Call findEnvironment(appEnv, appBlk)

If Workbooks(appEnv).Worksheets(appBlk).Range("xlasInvert").Value = 1 Then defCol1 = vbBlack: _
defCol2 = vbWhite: CTRLBOX.CtrlBoxWindow.ForeColor = defCol1: CTRLBOX.CtrlBoxWindow.BackColor = defCol2: Exit Sub
If Workbooks(appEnv).Worksheets(appBlk).Range("xlasInvert").Value = 2 Then defCol1 = vbWhite: _
...
'CTRLBOX.WindowSel.AddItem ("xlFlowStrip              Ctrl+F")
...
End Sub
Public Function OptionSelect(xBtn)
