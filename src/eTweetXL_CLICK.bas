...
Do Until EOF(1)
Line Input #1, xStr
ETWEETXLPOST.PostBox.Value = ETWEETXLPOST.PostBox.Value & xStr & vbNewLine
    Loop
        Close #1
            Call eTweetXL_CLICK.SavePostBtn_Clk
            xArt = "<lib>xtwt;clr(-post);$": Call lexKey(xArt)
            Exit Sub
                    End If
    
If Range("xlasSilent").Value2 <> 1 Then
YesNo = MsgBox("Would you like to automatically save the imported text as a draft(s)?", vbYesNo, AppTag)
End If
...
xFile = fd.Show
    ...
    If xFile = "-1" Then
        ...
        YesNo2 = MsgBox("Would you like to enter a name for your draft?", vbYesNo, AppTag)
    
        If YesNo2 = vbYes Then
        xName = InputBox("Enter a name for your draft:", AppTag)
        ETWEETXLPOST.DraftBox.Value = xName
            End If
                End If
