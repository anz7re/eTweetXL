...
SetForm:
Err.Clear
If Range("xlasWinForm").Value2 <> 3 Then Range("xlasWinForm").Value2 = 3 Else Range("xlasWinForm").Value2 = 4
GoTo Retry
...
Call fndWindow(xWin)
xWin.Hide
M = MsgBox("Display eTweetXL?", vbOKOnly, AppTag)
    If M = vbOK Then
        Call xWin.Show
            End If
...
If Range("xlasWinForm").Value2 = 3 Then
...
If Range("xlasWinForm").Value2 = 1 Then ETWEETXLHOME.Hide
If Range("xlasWinForm").Value2 = 1 Then ETWEETXLHOME.Show
...
'//Check xlFlowStrip Window...
If Range("xlasWinForm").Value2 = 3 Then Set oMedLinkBox = ETWEETXLPOST.MedLinkBox
If Range("xlasWinForm").Value2 = 4 Then Set oMedLinkBox = ETWEETXLQUEUE.MedLinkBox
...
End Function
Public Function AddRuntimeBtn_Clk(ByVal xPos As String) As String
...
End Function
Public Function DynOffset_Clk(ByVal xPos As String) As String
...
If Range("xlasWinForm").Value2 = 1 Then GoTo FlBarHome
If Range("xlasWinForm").Value2 = 2 Then GoTo FlBarSetup
If Range("xlasWinForm").Value2 = 3 Then GoTo FlBarPost
If Range("xlasWinForm").Value2 = 4 Then GoTo FlBarQueue
...
End Function
Public Sub AddLinkBtn_Clk(ByVal xPos As String)
...
lastRAL = Cells(Rows.Count, "AL").End(xlUp).Row '//apiLink for drafts
...
If xPos > 0 Then
    ...
    Range("ProfileLink").Offset(xPos, 0).Value = Range("Profile").Value2
    Range("Draftlink").Offset(lastRP, 0).Value = ETWEETXLPOST.DraftBox.Value
        Exit Sub
                ...
                '//Check pin
                 If Range("xlasSilent") <> 1 Then msg = InputBox("Re-enter pin:", AppTag)
                    ...
                    '//CHheck editing mode...
                        ...
                        Range("Scure").Offset(X, 0).Value = "***"
                        Range("Target").Offset(X, 0).Value = PinHldr '//RECORD
                        End If
                            Else
                        '//wrong pin entered...
...
xPass = Range("F1").Offset(X, 0).Value '//Get pass position
...
End Function
Public Sub AddUserBtn_Clk(xPos)
...
If xPos > 0 Then
    ...
    Range("TargetLink").Offset(lastRAL, 0).Value = Range("F" & xNum).Value
    End If
    Next
...
If Range("xlasWinForm").Value2 = 3 Then xTwt = xWin.DraftBox.Value: Call eTweetXL_POST.pstDraftData(xTwt) Else _
...
If Range("xlasWinForm").Value2 = 3 Then xWin.DraftBox.Value = xTwt
...
'//Check for pass...
If Range("xlasWinForm").Value2 = 2 Then
...
Dim iDraft As String: Dim iTime As String: Dim iUser As String: Dim NwTwtFile As String
...
xArt = "<lib>xbas;wastetime(3);$": Call lexKey(xArt)
    End If
...
iDraft = oDraftBox.List(xPos - 1)
iDraft = Replace(iDraft, "(" & xPos & ") ", vbNullString)
DraftArr(xPos) = iDraft
If Range("xlasSilent").Value2 <> 1 Then
'//Update connection state (xlFlowStrip)...
ETWEETXLPOST.xlFlowStrip.Value = "Connecting drafts: " & (xPos * 100) / xTotal & "%..."
xArt = "<lib>xbas;wastetime(1);$": Call lexKey(xArt)
    End If
        Next
...
iTime = oRuntimeBox.List(xPos - 1)
iTime = Replace(iTime, "(" & xPos & ") ", vbNullString)
iTime = Replace(iTime, " ", vbNullString)
RuntimeArr(xPos) = iTime
'//Set runtime(s)
Range("Runtime").Offset(xPos, 0).Value2 = iTime
If Range("xlasSilent").Value2 <> 1 Then
'//Update connection state (xlFlowStrip)...
...
iUser = oUserBox.List(xPos - 1)
iUser = Replace(iUser, "(" & xPos & ") ", vbNullString)
Range("UserLink").Offset(xPos, 0).Value2 = iUser
If Range("xlasSilent").Value2 <> 1 Then
'//Update connection state (xlFlowStrip)...
...
If Range("xlasWinForm") = 3 Then Call eTweetXL_GET.getPostData(xType)
    ElseIf xFil = 0 Then
    xType = 0
